...
function reload() {
   $('#player').show();
   $.getJSON("/video/context", null, function( context ) {
        	if( context ) {
			user = context.user;
...
$(document).ready(function() {
        $("#menu-catalog").click(function() {
            $("#catalog-sub-menu").css({ left: "25%"} );
            $("#content").animate({ width: "toggle"}, "fast" ); //slideToggle("fast");
        });

	$("#bag-toggle").click(function(){ 
	    $("#bag-content").slideToggle("fast"); 
        });

	$('#menu-catalog').ajaxError(function(e, r, settings){
