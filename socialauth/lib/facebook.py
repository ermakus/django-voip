...
def get_user_info(api_key, api_secret, cookies):
    user_info_params = {
                                        ...
                                        'fields': 'uid,first_name,last_name,pic_small,name,current_location,sex,email',
                                        'format': 'json',
                                    }
    ...
    user_info_response  = simplejson.load(urllib2.urlopen(REST_SERVER, user_info_params))
...
def talk_to_fb(api_key, api_secret, params):
    ...
    response = simplejson.load(urllib2.urlopen(REST_SERVER, data))
    return response
